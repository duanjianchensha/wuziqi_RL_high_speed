...
public:
               ...
               float temp = 1e-3f)
    {
        ...
        if (temp < 1e-6f) {
            ...
            float sum = 0.f;
            for (int i = 0; i < n; ++i) {
                probs_out[i] = std::pow(visits[i], 1.f / temp);
                sum += probs_out[i];
            }
            for (int i = 0; i < n; ++i) probs_out[i] /= sum;
        }
        return n;
    }
    ...
    void _playout(gomoku::Board& board, MCTSNode* node) {
        ...
        while (!node->is_leaf()) {
            ...
            for (auto& [a, child] : node->children) {
                ...
                if (v > best_val) { best_val = v; best_action = a; best_child = child; }
            }
            if (!best_child) break;
        ...
        if (!board.game_over()) {
            ...
            auto w = board.winner;
            if (*w == 0) leaf_value = 0.f;
            else leaf_value = (*w == static_cast<int>(board.current)) ? -1.f : 1.f;
        }
